...
export const NeeiPresentation: React.FC<NeeiPresentationProps> = ({ cards, onNotify }) => {
  ...
  const qrSize = isFullscreen ? (screenHeight < 760 ? 180 : screenHeight < 880 ? 205 : 250) : 210;

  // Pure presentation styling with dark mode support
  const pageBg =
    'bg-[#f0f7fa] dark:bg-[#070d14] text-[#1d1c1c] dark:text-slate-100 transition-colors duration-300';
  const cardBg =
    'bg-white dark:bg-[#0c1421] border border-[#b6ccd8]/70 dark:border-slate-800 shadow-lg shadow-cyan-900/5 dark:shadow-black/40 text-[#1d1c1c] dark:text-slate-100';
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans ${pageBg}`}>
      ...
      <div
        ref={slideRef}
        className={`flex-1 flex flex-col items-center ${pageBg} ${
          isFullscreen
      ...
      >
        ...
        <div
          className={`w-full ${isCompactLaptop ? 'space-y-5' : 'space-y-7 sm:space-y-8'} max-w-5xl my-auto`}
        >
          ...
          <div className={`text-center ${isCompactLaptop ? 'space-y-2' : 'space-y-3'}`}>
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  (e.target as HTMLImageElement).src =
                    '/assets/logoneeigrande-removebg-preview.png';
                }}
              />
            ...
            <h1
              className={`${isCompactLaptop ? 'text-xl sm:text-2xl lg:text-3xl' : 'text-2xl sm:text-3xl lg:text-4xl'} font-black tracking-tight leading-tight m-0 text-slate-900 dark:text-white`}
            >
              Junta-te aos Nossos Canais
            </h1>
            <p className="text-xs sm:text-sm max-w-lg mx-auto text-slate-600 dark:text-slate-400">
              Aponta a câmara do teu telemóvel para qualquer um dos QR Codes abaixo para acederes
              aos grupos e redes oficiais.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 w-full items-stretch">
            {displayCards.map((card, index) => {
              ...
              if (card.category === 'whatsapp' || card.title.toLowerCase().includes('whatsapp')) {
                ...
                badgeColor =
                  'bg-gradient-to-tr from-amber-500 via-pink-600 to-purple-600 text-white';
              } else if (
                card.category === 'discord' ||
                card.title.toLowerCase().includes('discord')
              ) {
              ...
              return (
                ...
                >
                  {/* Category Pill */}
                  <div
                    className={`w-full flex items-center justify-start ${isCompactLaptop ? 'mb-2' : 'mb-3'}`}
                  >
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm ${badgeColor}`}
                    >
                  ...
                  <div
                    className={`${isCompactLaptop ? 'p-2.5 sm:p-3' : 'p-3.5 sm:p-4'} bg-white rounded-2xl shadow-inner border border-slate-200 dark:border-slate-700/60 flex items-center justify-center ${isCompactLaptop ? 'my-0.5' : 'my-1'}`}
                  >
                  ...
                  {/* URL / Action */}
                  <div
                    className={`w-full ${isCompactLaptop ? 'mt-3 space-y-1.5' : 'mt-4 space-y-2'}`}
                  >
          ...
          <div
            className={`relative overflow-hidden rounded-3xl ${isCompactLaptop ? 'p-3.5 sm:p-4' : 'p-5 sm:p-6'} border transition-all duration-300 shadow-md bg-gradient-to-r from-white via-[#f0f7fa] to-[#d4eaf7]/80 dark:from-[#0c1421] dark:via-[#0c1421] dark:to-cyan-950/40 border-[#b6ccd8] dark:border-slate-800`}
          >
          ...
          <div className="text-center pt-2 text-[11px] text-slate-400 dark:text-slate-500">
            <span>
              Núcleo de Estudantes de Engenharia Informática &bull; Universidade do Algarve
            </span>
          </div>
        </div>
      </div>
