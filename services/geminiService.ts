...
export const analyzeCodeWithGemini = async (params: SimulationParams) => {
  ...
  const primaryTestCase = testCases && testCases[0];

  let realOutput = '';
  let realError = '';

  if (!primaryTestCase) {
    // Sem caso de teste definido - pedimos ao backend para analisar
    realError = 'Nenhum caso de teste fornecido.';
  } else {
    ...
    if (pistonResult) {
      ...
      if (pistonResult.compile && pistonResult.compile.stderr) {
        ...
        if (pistonResult.run.code !== 0) {
          realError =
            pistonResult.run.stderr || `Erro de execução (Exit Code: ${pistonResult.run.code})`;
        } else if (pistonResult.run.stderr) {
          // Às vezes há avisos no stderr mesmo com sucesso, mas assume-se como info
        ...
        if (!realError && cleanOutput === cleanExpected) {
          return {
