          ...
          <div className="space-y-3">
            {COURSES.map((course) => (
              <div key={course.id}>
                ...
                >
                  {course.shortName}
                  <span className="text-[10px] font-mono opacity-80 bg-white/20 px-1 rounded">
                    {course.language}
                  </span>
                </button>
                ...
                {selectedCourse.id === course.id && (
                  <div className="ml-3 mt-2 pl-3 border-l-2 border-primary-200 dark:border-cyan-900/50 space-y-1">
                    {course.exercises.map((ex) => (
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${selectedExercise.id === ex.id ? 'bg-accent-200 dark:bg-cyan-400' : 'bg-gray-300 dark:bg-slate-600'}`}
                        ></div>
        ...
        <div className="bg-white dark:bg-[#0c1724] p-5 rounded-xl shadow-sm border border-primary-200 dark:border-cyan-900/50 flex flex-col gap-3">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-text-200 dark:text-slate-300 text-sm mt-1">
          ...
          <div className="flex items-center gap-4 text-xs text-text-200 dark:text-slate-400 border-t border-gray-100 dark:border-slate-800 pt-3 mt-1">
            <span className="flex items-center gap-1 bg-gray-100 dark:bg-slate-800 px-2 py-1 rounded">
              <Code2 size={12} /> {selectedCourse.language === 'c' ? 'GCC 10.2' : 'OpenJDK 15'}
            </span>
            <span className="flex items-center gap-1 bg-gray-100 dark:bg-slate-800 px-2 py-1 rounded">
              <Clock size={12} /> 30s Timeout
            </span>
            <span className="flex items-center gap-1 bg-gray-100 dark:bg-slate-800 px-2 py-1 rounded">
              <ShieldAlert size={12} /> Sandbox (Piston)
            </span>
          </div>
        </div>
            ...
            <div className="flex border-b border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-[#081320]">
              <button
                ...
                className={`px-4 py-3 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors ${
                  activeTab === 'terminal'
                    ? 'border-accent-200 dark:border-cyan-400 text-accent-200 dark:text-cyan-400 bg-white dark:bg-[#0c1724]'
              ...
              >
                ...
                {status !== ExecutionStatus.IDLE && status !== ExecutionStatus.RUNNING && (
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded-full ${status === ExecutionStatus.SUCCESS ? 'bg-green-100 dark:bg-green-950/70 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-950/70 text-red-700 dark:text-red-300'}`}
                  >
              ...
              {hint && (
                <button
                  ...
                  className={`px-4 py-3 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors ${
                    activeTab === 'hint'
                      ? 'border-yellow-400 text-yellow-600 dark:text-yellow-400 bg-white dark:bg-[#0c1724]'
                      : 'border-transparent text-yellow-600 dark:text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-950/30'
                  }`}
                >
                  <Lightbulb size={16} className={activeTab === 'hint' ? 'fill-yellow-400' : ''} />{' '}
                  AI Tutor
                </button>
              )}
            </div>
              ...
              {activeTab === 'terminal' && (
                <div className="p-4 font-mono text-sm text-gray-300 min-h-full">
                  {status === ExecutionStatus.IDLE ? (
                    ...
                    <>
                      <div className="text-gray-500 text-xs mb-2 select-none">
                        $ {selectedCourse.language === 'c' ? './main' : 'java Main'}
                      </div>
                      <div className="whitespace-pre-wrap">
                        {output || (
                          <span className="text-gray-600 italic">(Sem output no stdout...)</span>
                        )}
                      </div>
                      {status === ExecutionStatus.SUCCESS && (
                        <div className="text-green-500 text-xs mt-4 select-none">
                          Process finished with exit code 0
                        </div>
                      )}
                      {status !== ExecutionStatus.SUCCESS && status !== ExecutionStatus.RUNNING && (
                        <div className="text-red-500 text-xs mt-4 select-none">
                        ...
                        className={`p-4 rounded-lg flex items-start gap-3 ${
                          status === ExecutionStatus.SUCCESS
                            ...
                            : status === ExecutionStatus.TIMEOUT
                              ? 'bg-orange-50 dark:bg-orange-950/40 text-orange-800 dark:text-orange-300 border border-orange-200 dark:border-orange-800'
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {selectedExercise.testCases.map((tc, idx) => (
                            <div
                              key={idx}
                              className="border border-gray-200 dark:border-slate-800 rounded-lg p-3 text-sm"
                            >
                              <div className="grid grid-cols-2 gap-4 mb-2">
                                <div>
                                  ...
                                  <div className="font-mono bg-gray-100 dark:bg-[#070d14] p-1.5 rounded text-gray-800 dark:text-slate-200 mt-1 border border-transparent dark:border-slate-800">
                                    {tc.input || '(vazio)'}
                                  </div>
                                </div>
                                <div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="mt-6 text-sm text-yellow-700/60 italic border-t border-yellow-200 pt-2">
                        Esta dica foi gerada automaticamente pelo Gemini analisando o erro real do
                        teu código.
                      </div>
                    </div>
                  </div>
