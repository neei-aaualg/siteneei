...
export function buildDiscordUrl(invite: string): string {
  const cleanInvite = invite.trim().replace(/^(https?:\/\/)?(www\.)?(discord\.gg\/|discord\.com\/invite\/)/, '')
  if (!cleanInvite) return ''
  return `https://discord.gg/${cleanInvite}`
}

export function buildWifiString(ssid: string, password: string, encryption: 'WPA' | 'WEP' | 'nopass' = 'WPA'): string {
